...
const TableVideoCategory: React.FC<HeaderNavWorkoutVideosProps> = ({
  onClose,
}) => {
  const [selectedIdPerGroup, setSelectedIdPerGroup] = useState<{
  ...
  const groupCategoriesByName = (
    ...
    sortedGroupNames.forEach((name) => {
      sortedGroups[name] = grouped[name].sort((a, b) =>
  ...
  const handleFilter = () => {
    if (!selectedIdPerGroup) {
      ...
      const categoryIds = Object.keys(selectedIdPerGroup)
        .map((key) => `categoryId${selectedIdPerGroup[key]}`)
        .join("");
      ...
      <div className="flex flex-col m-auto max-w-7xl">
        <div className="grid gap-4 w-full grid-cols-6">
          {Object.entries(groupedCategories).map(([name, categories]) => (
            <div
              key={name}
              className="flex flex-col border-r border-gray-300 last:border-r-0 px-4 my-6"
            >
              <div className="flex items-center py-2">
                <h1 className="text-xs uppercase text-[#868A93] font-bold">
                  {name}
                </h1>
              </div>
              {categories.map((category) => (
                <div key={category.id} className="flex items-center py-1">
                  ...
                  >
                    ...
                    {selectedIdPerGroup[name] === category.id && (
                      <Check color="#7065cd" width={18} height={18} />
                    )}
                  </label>
                </div>
              ))}
        ...
        <div className="flex justify-between w-full items-center px-4 py-6">
          ...
          <div>
            <span className="pr-2 cursor-pointer" onClick={onClose}>
              Cancel
            </span>
