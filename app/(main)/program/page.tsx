...
import {
  fetchLatestWorkoutProgramData,
  fetchWorkoutProgramData,
...
const ProgramPage = () => {
  ...
  useEffect(() => {
    const getWorkoutProgram = async () => {
      const [workoutProgram, topics] = await Promise.all([
        fetchLatestWorkoutProgramData(),
        fetchAllTopic(),
      ]);
      ...
      <div className="flex w-full gap-5 pb-5 pt-8">
        <div className="flex w-[60%] justify-between">
          {loading ? (
            ...
            <>
              <span className="text-[22px] font-semibold leading-7">
                Latest Challenges
              </span>
              <div>
      ...
      <div className="grid grid-cols-5 gap-4 h-[430px]">
        {loading ? (
          ...
          <>
            {firstProgram && (
              <Link
                className="flex w-full bg-white rounded-2xl px-[15px] py-[47px] gap-3 col-span-2"
                href={`/program/${firstProgram.id}`}
                key={firstProgram.id}
              >
                <img
                ...
                <div className="flex flex-col gap-3 relative w-[48%]">
                  <span className="text-[#868A93] font-medium text-[12px] leading-3">
                    {firstProgram.releaseDate
                      ? formatDate(firstProgram.releaseDate)
                      : ""}
                  </span>
                  ...
                  <div className="flex gap-1">
                    <div className="flex gap-1 text-center items-center px-1 py-2 rounded-full bg-[#F7F7F7]">
                      ...
                      <span className="text-[12px]">{firstProgram.day}</span>
                    </div>
            ...
            {workoutPrograms.slice(1, 4).map((program) => (
              <Link href={`/program/${program.id}`} key={program.id}>
                <ProgramCard
                  ...
                  img={program.img}
                  releaseDate={
                    program.releaseDate ? formatDate(program.releaseDate) : ""
                  }
                />
              </Link>
            ))}
